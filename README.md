<h1 align="center">
  <img
    src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg"
    width="20"
    alt="FireHub Icon"
  />
  The FireHub Project — GitHub Configurations and Community Health Files<br>
  <a href="./LICENSE">
    <img
      src="https://img.shields.io/badge/License-MIT-d50000.svg?style=flat&labelColor=30363d&logo=data:image/svg+xml;base64,"
      alt="GitHub license"
  /></a>
</h1>

<p>
  <a href="https://github.com/The-FireHub-Project/.github">
    <img
      src="https://img.shields.io/github/created-at/The-FireHub-Project/.github?style=flat&logo=github&logoColor=959da5&label=created%20at&labelColor=31373e"
      alt="GitHub created at"
  /></a>
  <a href="https://github.com/The-FireHub-Project/.github/commits/master/">
    <img
      src="https://img.shields.io/github/last-commit/The-FireHub-Project/.github/master?style=flat&logo=github&logoColor=959da5&label=last%20commit%20(branch)&labelColor=31373e"
      alt="GitHub last commit (branch)"
  /></a>
  <a href="https://github.com/The-FireHub-Project/.github/commits/master/">
    <img
      src="https://img.shields.io/github/commit-activity/m/The-FireHub-Project/.github/master?style=flat&logo=github&logoColor=959da5&label=commit%20activity%20(branch)&labelColor=31373e"
      alt="GitHub activity (branch)"
  /></a>
  <a href="https://github.com/The-FireHub-Project/.github/graphs/contributors/">
    <img
      src="https://img.shields.io/github/contributors/The-FireHub-Project/.github?style=flat&logo=github&logoColor=959da5&label=contributors&labelColor=31373e"
      alt="GitHub contributors"
  /></a>
  <a href="https://github.com/The-FireHub-Project/.github/">
    <img
      src="https://img.shields.io/github/languages/count/The-FireHub-Project/.github?style=flat&logo=data:image/svg+xml;base64,&logoColor=959da5&label=language%20count&labelColor=31373e"
      alt="GitHub language count"
  /></a>
  <a href="https://github.com/The-FireHub-Project/.github/">
    <img
      src="https://img.shields.io/github/languages/top/The-FireHub-Project/.github?style=flat&logo=data:image/svg+xml;base64,&logoColor=959da5&label=top%20language&labelColor=31373e"
      alt="GitHub top language"
  /></a>
</p>

This repository defines the shared GitHub configuration and governance standards for the FireHub ecosystem.

It centralizes organizational workflows, contribution rules, issue and pull request templates, and other repository-wide automation used across all FireHub projects.

---

## <img src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg" width="15" alt="FireHub Icon"> Repository Purpose

This repository contains **organization-wide GitHub configurations, workflows, issue templates, and community standards** for The FireHub Project.

It is used to centralize and standardize:

- GitHub Actions workflows for CI/CD
- Issue and pull request templates
- Branch protection rules and rulesets
- Community health files (CODE_OF_CONDUCT.md, CONTRIBUTING.md)
- Badges and status indicators
- Security policies

## <img src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg" width="15" alt="FireHub Icon"> Community

- Be sure to review the [Code of Conduct](https://github.com/The-FireHub-Project/.github/blob/master/.github/CODE_OF_CONDUCT.md).
- Read a [Contributing](https://github.com/The-FireHub-Project/.github/blob/master/.github/CONTRIBUTING.md) file to see how you can contribute to this project.
- Read our [Security policy](https://github.com/The-FireHub-Project/.github/blob/master/.github/SECURITY.md) file to see how you can report a vulnerability.
- Review the project [Governance model](https://github.com/The-FireHub-Project/.github/blob/master/.github/GOVERNANCE.md), including roles, decision-making, RFC process, and releases.

## <img src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg" width="15" alt="FireHub Icon"> Authors and Contributors

- Project authors can be found [here](https://github.com/The-FireHub-Project/.github/blob/master/.github/AUTHORS.md).
- The full list of individual contributors can be found [here](https://github.com/The-FireHub-Project/.github/blob/master/.github/CONTRIBUTORS.md).

## <img src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg" width="15" alt="FireHub Icon"> Brand & Trademark

Please refer to [TRADEMARK.md](https://github.com/The-FireHub-Project/.github/blob/master/TRADEMARK.md) and [BRAND.md](https://github.com/The-FireHub-Project/.github/blob/master/BRAND.md) for guidelines on FireHub branding.

## <img src="https://raw.githubusercontent.com/The-FireHub-Project/the-firehub-project.github.io/master/resources/graphics/icons/firehub.svg" width="15" alt="FireHub Icon"> License

This software is licensed under the MIT License.

For more details, read the full license [here](./LICENSE).